# 🚀 Variables de Entorno Completas para KMV
...
- `OAUTH_REDIRECT_URI` debe ser la URL pública de tu bot en Railway (Railway te da una URL automáticamente)
...
## 🔧 Variables TÉCNICAS (Opcionales - Railway las configura automáticamente)

Estas variables Railway las configura automáticamente, pero puedes configurarlas manualmente si lo necesitas:
...
**Nota**: Railway configura `PORT` automáticamente. Solo configura `OAUTH_PORT` si necesitas un puerto específico para OAuth2.
...
| `OAUTH_REDIRECT_URI` | 🟠 | **Sí (OAuth2)** | URL de callback OAuth2 | `https://tu-app.up.railway.app/oauth/callback` |
...
- `OAUTH_REDIRECT_URI` debe ser la URL pública de Railway (Railway te la da automáticamente)
