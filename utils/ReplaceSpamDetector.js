...
const REPLACE_SPAM_TIME_WINDOW = 5000; // 5 segundos (ajustable entre 3-5)
...
export class ReplaceSpamDetector {
   ...
   * @returns {Object} { isSpam: boolean, count: number, timeWindow: number }
   */
  static checkReplaceSpam(userId) {
    const now = Date.now();
